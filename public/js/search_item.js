...
export class SearchItemView {
    ...
    setState(newState) {
        if (newState.isChecked !== undefined)
            this.isChecked = newState.isChecked;
    ...
    registEventHandler() {
        document
            .querySelector(`.search_bar_item.${this.className}`)
            .addEventListener("click", this.toggleCheckItem.bind(this));
    }
    toggleCheckItem() {
